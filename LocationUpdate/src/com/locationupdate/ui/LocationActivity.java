...
public class LocationActivity extends Activity implements ConnectionCallbacks,
	...
	/**
	 * Ensures that only one button is enabled at any time. The Start Updates
	 * button is enabled if the user is not requesting location updates. The
	 * Stop Updates button is enabled if the user is requesting location
	 * updates.
	 */
	...
	// mLastUpdateTimeTextView.setText("Updated Time : " + mLastUpdateTime);
	// }
	...
	public void onLocationChanged(Location location) {
		...
		latitude.add(String.valueOf(mCurrentLocation.getLatitude()));
		longitude.add(String.valueOf(mCurrentLocation.getLongitude()));
		updateTime.add(mLastUpdateTime);
		...
		File file = new File(myDir, "latlng.txt");

		try {
			FileOutputStream fOut = new FileOutputStream(file);

			for (int i = 0; i < latitude.size(); i++) {
				String out = "Latitude : " + latitude.get(i) + " Longitude : "
						+ longitude.get(i) + " Updated Time : "
						+ updateTime.get(i) + "\n";
